  ...
  return (
    <div className='flex justify-between items-center h-20 text-white bg-black w-full px-4 fixed'>
        ...
        <ul className='hidden md:flex'>
            {links.map(({id,link})=>(
                <li key={id} className='px-4 text-gray-500 capitalize hover:scale-105 cursor-pointer font-medium duration-200'>{link}</li>
            ))}
        </ul>

        <div className='text-gray-500 cursor-pointer pr-4 z-10 md:hidden' onClick={()=>setNav(!nav)}>
        ...
        {links.map(({id,link})=>(
                <li key={id} className='text-4xl capitalize cursor-pointer px-4 py-6'>{link}</li>
            ))}
        </ul>
        )}
    </div>
